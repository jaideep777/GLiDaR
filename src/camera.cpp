...
//	if (activeCamera == this) this->sortShapes();
...
//	float w = glutGet(GLUT_WINDOW_WIDTH);
	...
	if (lClick){
		activeCamera->rx += 0.02*(y - y0);
		activeCamera->ry += 0.02*(x - x0);
	}
	else if (mClick || (rClick && state == GLFW_PRESS)){
		activeCamera->ty -= (1/250.f)*(y - y0);	// -= because y is measured from top
		activeCamera->tx += (1/250.f)*(x - x0);
	}
	else if (rClick){
